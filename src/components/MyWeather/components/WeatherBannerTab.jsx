...
class WeatherBannerTab extends React.Component {
  static renderTabPanel(item, unit, locale) {
    const localeRegion = locale || "zh-tw";
    return (
      <TabPanel key={`tp${item.dt}`}>
        <WeatherBanner data={item} unit={unit} locale={localeRegion} />
      </TabPanel>
    );
  }
  ...
  constructor(props) {
    ...
    forecastOfDay.forEach((item, index) => {
      this.marks[index] = item.moment.locale(locale).format(" h:mm a");
    });
      ...
      <Container>
        ...
        <Tabs selectedIndex={tabIndex} onSelect={false}>
          {forecastOfDay.map(item =>
            WeatherBannerTab.renderTabPanel(
              item,
              this.props.unit,
              this.props.locale
            )
